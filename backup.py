...
class GradioApp:
    ...
    def image_segmentation_ui(self):
        ...
        with gr.Row() as segment_block:
            ...
            return input_image, output_image, clear_button, submit_button, segment_block
    ...
    def get_task_interface_page(self):
        with gr.Blocks() as task_block:
            ...
            task_output = gr.Column()

            # Define UI components globally
            input_image, output_image, clear_button, submit_button, segment_block = self.image_segmentation_ui()

            def update_task_ui(task_name):
                if task_name == "Image Segmentation":
                    return segment_block
                elif task_name == "Image Sharpness":
                    return gr.Markdown("## Image Sharpness Coming Soon!")
                else:
                    return gr.Markdown("## Unknown task selected")

            task_dropdown.change(fn=update_task_ui, inputs=[task_dropdown], outputs=[task_output])

            # Attach button events separately
            submit_button.click(fn=self.segmentation.process_image, inputs=[input_image], outputs=[output_image])
            clear_button.click(fn=self.segmentation.reset, inputs=[], outputs=[input_image, output_image])
